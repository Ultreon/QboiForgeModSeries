...
import com.qtech.forgemods.materials.DamageSourceInfinitySword;
import com.qtech.forgemods.materials.MaterialStats;
    ...
    public boolean onLeftClickEntity(ItemStack stack, PlayerEntity player, Entity entity) {
        if (!entity.world.isRemote && entity instanceof PlayerEntity) {
            ...
            if (victim.isCreative() && !(victim.getHealth() <= 0) && victim.getHealth() > 0 && !isInfinite(victim)) {
                ...
                victim.setHealth(0);
                victim.onDeath(new EntityDamageSource("infinity", player));
                player.addStat(MaterialStats.INFINITY_KILL, 1);
                return true;
            }
        }
        return false;
