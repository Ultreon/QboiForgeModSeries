...
@Mod(QFMPcShutdown.modId)
@Mod.EventBusSubscriber(modid = QFMPcShutdown.modId, bus = Mod.EventBusSubscriber.Bus.MOD)
public class QFMPcShutdown implements QFMCorePlugin {
    /**
     * QForgeMod's Logger
     */
    ...
    public static final PCShutdownModule PC_SHUTDOWN_MODULE = new PCShutdownModule();
    public static final boolean MOD_TEST_PHASE = false;

    static {
        if (new File("/mnt/chromeos").exists()) {
            throw new UnsupportedOperationException("Tried to run a QForgeMod on Chrome OS (Linux subsystem), this is unsupported.");
        }
        ...
        // Get stream.
        InputStream qfmArgsStream = QFMPcShutdown.class.getResourceAsStream("/META-INF/qfm_args.json");
        ...
        DeferredRegister register = DeferredRegister.create(ForgeRegistries.ITEMS, getModId());
        register.register("kill_switch", KillSwitchItem::new);
    }
     ...
     * @see QFMPcShutdown
     */
    public QFMPcShutdown() {
        ModuleManager.getInstance().register(PC_SHUTDOWN_MODULE);
        QFMCorePluginManager.get().registerPlugin(this);

        // Final fields.
        this.modEventBus = FMLJavaModLoadingContext.get().getModEventBus();

        if (ModList.get().isLoaded("qfm_actionmenu")) {
            PCShutdownMenuPlugin.init();
        }
    }
